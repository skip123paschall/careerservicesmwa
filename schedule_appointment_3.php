...
require_once 'includes/page_start_schd_appt_3.php';
...
<div data-role="page" data-content-theme="a" id="csmwa">
	...
	<?php if ($_SERVER['REQUEST_METHOD'] !== 'POST') { 
			...
			$phone = $_SESSION['_phone']; ?>
					...
					<form class="validate" data-ajax="false" method="POST" id="appointment_form" name="appointment_form">
						...
						<div class="ui-field-contain" id="appointment">								
							<label for="appointment_type" class="select">Appointment type</label> 
							<select name="appointment_type" id="appointment_type" data-native-menu="false">
								<option></option>
								<option value="Career Assessment/Exploration"/>Career Assessment/Exploration</option>
								<option value="Resume/Cover Letter  Assistance">Resume/Cover Letter Assistance</option>
								<option value="Job and Internship Search">Job/Internship Search</option>								
								<option value="Mock Interview Prep">Mock Interview Prep</option>
								<option value="NEworks Registration">NEworks Registration</option>									
								<option value="InternNE Registration">InternNE Registration</option>	
								<option value="Other">Other</option>
							</select>
						</div>							
	...
	<?php } else {
		...
		if (isset($_POST['appointment_type'])) {
			...
			$appointment_type = 'None Specified';
		}			
		...
		if (count($errors) === 0) {  
			...
			$body = $firstName . ' ' . $lastName . " has requested to learn more about career opportunities.\n\n" .
				...
				"This appointment request came from the Career Services home page.\n\n" .					
			...
			$body = $firstName . ' ' . $lastName . " has requested to meet with a Career Services Navigator.\n\n" .
				...
				"The appointment type " . $appointment_type . ".\n\n" .					
				"Preferred Contact Method: " . $contactMethod . "\n" .
				"Student Email Address: " . $email . "\n" .
				"Student Phone Number: " . $phone . "\n" .
			...
			if ($intended_major == "Information Technology") {	
				$recipient = "bstegeman@mccneb.edu"; 		
				//$recipient = "cookiepaschall@gmail.com"; 
			}else{
				$recipient = "careerservices@mccneb.edu";  
				//$recipient= "skip123paschall@gmail.com";  				
			}
			...
			$body = "Thank you " . $firstName . " " . $lastName . " for using the MCC Career Services app.\n\n" .
				...
				"Your Appointment Type: " . $appointment_type . "\n" .	
				"Please do not reply to this message directly.\n\n";
			}else{
			...
			try {				
				$query = 'INSERT INTO appt_request(visit_num, first_name, last_name, email_addr, type_1, type_2, type_3, mcc_status, contact_method, phone, mtg_days, mtg_time, mtg_location, status, intended_major, appointment_type)' .
					'VALUES(:visit_num, :first_name, :last_name, :email_addr, :type_1, :type_2, :type_3, :mcc_status, :contact_method, :phone, :mtg_days, :mtg_time, :mtg_location, :status, :intended_major, :appointment_type)';
				...
				$statement->bindParam(':appointment_type', $appointment_type, PDO::PARAM_STR);					
